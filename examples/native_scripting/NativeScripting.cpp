...
namespace NativeScripting
{
    ...
    void Example::spawnCircle(const CircleConfig& config) {
        ...
        e->add<CNativeScript>([this](Entity& entity) { onBoundingCircleCollision(entity); });
    }
    ...
    void Example::spawnRectangle(const RectangleConfig& config) {
        ...
        e->add<CNativeScript>([this](Entity& entity) { onBoundingBoxCollision(entity); });
    }

    // Scripts
    void Example::onBoundingBoxCollision(Entity& entity) {
        auto wSize = m_gameEngine->renderTarget().getSize();
        auto& cTrans = entity.get<CTransform>();
        auto& cCollider = entity.get<CBoundingBox>();
    ...
    void Example::onBoundingCircleCollision(Entity& entity) {
        auto wSize = m_gameEngine->renderTarget().getSize();
        auto& cTrans = entity.get<CTransform>();
        auto& cCollider = entity.get<CBoundingCircle>();
